...
const SIBLINGS = [
  ...
  { href: "/_/pages/blue", name: "Blue", active: true },
  { href: "/_/pages/green", name: "Green", active: false },
] as const;

export default function PagesBlue() {
  return (
    <div class="relative flex min-h-[100svh] flex-col bg-blue-500 pt-24 text-white">
      <Title>Blue · page transition</Title>
      <TransitionBadge />

      <div class="flex flex-1 flex-col justify-between px-gx py-12">
        <div>
          <p class="font-mono text-sm uppercase tracking-widest text-blue-100/80">
            Incoming branch
          </p>
        ...
        <nav class="flex flex-wrap gap-3">
          {SIBLINGS.map((link) => (
            <A
              ...
              classList={{
                "bg-white text-blue-600 border-white": link.active,
              }}
            >
              {link.name}
            </A>
